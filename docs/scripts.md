...
| `fm-teardown.sh`         | Fail-closed teardown: return landed ship worktrees, require completed scout deliverables, retire secondmate homes, plus the `reconcile-reassigned-slot` repair that detaches a stale record from a re-lent pool slot |
